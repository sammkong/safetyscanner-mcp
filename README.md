...
> 거래·알바 글을 붙여 넣으면 **사기 의심 신호**를 탐지해  
> **위험도 점수, 근거 문구, 위험한 이유, 안전 체크리스트**를 알려주는 읽기 전용 MCP 서버입니다.

SafetyScanner는 사용자가 중고거래 글이나 단기알바 공고를 보고  
**“이거 괜찮은가?”** 를 판단하기 전에 호출할 수 있는 **안전 필터**입니다.

중고거래·단기알바에서 자주 쓰이는 **선입금 요구, 외부 메신저 유도, 자금 이체·대리 인출, 불법 장비 설치, 명의·계정 대여** 같은 범죄 가담형 패턴을 탐지합니다.

에이전트가 글을 대신 검토할 때  
막연한 감이 아니라 **어떤 신호가 왜 위험한지**를 근거 문구와 함께 설명하고,  
지원·거래 전에 확인할 체크리스트까지 제공합니다.
